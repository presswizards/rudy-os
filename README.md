...
<img src="./docs/logo.png" alt="Rudy OS — agent harness that runs a whole office of agents" width="340">

# Rudy OS

### Agent harness that runs a whole office of agents

<a href="https://rudydoes.com"><img src="./docs/media/tour.gif" alt="Rudy OS tour: hiring an agent, handing Rudy a job, the floor clocking in" width="800"></a>

**[rudydoes.com](https://rudydoes.com)** · **[Download for macOS](https://github.com/nafdalex/rudy-os/releases/latest/download/Rudy.dmg)**
...
you already run into an always-on teammate, one that keeps working while you're away and
...
Agents that message, route, and remember, coordinated by **Rudy** and
...
> [!NOTE]
> **The world's best agents. All of them yours.**
> Rudy OS takes the terminal-agent CLIs you already run — `claude`, `agy`, `codex`, `grok`,
...
> office floor — and **Rudy** routes work between them while you watch. He's the
...
Rudy OS is a desktop app that wraps **real terminal-agent CLIs** as fully-capable agents,
wires them into a **hive mind**, and puts **Rudy** in charge — the one agent *you*
...
- **The hive coordinates them.** Agents read their memory and drain a mailbox; the router moves
  messages between inboxes; Rudy, the boss agent, adjudicates, assigns, and escalates only when it needs you.
...
```
            you ── talk to ──►  ┌─────────────┐
                                │    Rudy     │  the boss: orchestrator
                                │  (his desk) │  roster · routing · adjudication
...
3. **Rudy runs the floor** — he reads every request, resolves routine ones himself (keeping
...
- **A boss you talk to.** Rudy routes tasks, adjudicates traffic, and escalates only what needs a human.
...
- **Prerequisites** — one Settings page showing which supporting tools (uv, git, Node, MemPalace, each agent CLI) you have, what each is for, and a button that asks Rudy to install what is missing.

> [!NOTE]
> **Status: v0.5.0, the first shipping release under the Rudy OS name.** The rename goes all the
> way down: identity, palette, onboarding, the cast of fifteen hires, the site, and
...
spawn your first session — Rudy seats himself at the boss desk automatically.
...
- **Hive / event plane.** `hive.ts` is the on-disk multi-agent layer; `hooks.ts` runs the hook
  ...
  provider outboxes, Rudy adjudicates, and idle/inbox wakeups keep workers draining mail.
...
src/
  ...
  renderer/src/
    ...
    CommandCenterPanel,      Rudy's control surface (Terminal/Floor/Memory/Activity/Tasks/Triggers/Handbook tabs)
...
docs/                        `logo.png`, `banner.png`, landing page (GitHub Pages → rudydoes.com)
...
[`DESIGN.md`](./DESIGN.md) is canonical; every component derives from its tokens. The Rudy OS
brand layers a **deep maroon** (`#6E1423`) and **gold** (`#F4D35E`) on top for logo and
chrome. The 15 avatars are Rudy and his hires, differentiated by hair, skin and accessory recipes.

## Roadmap

Shipped through **v0.3.2** — twelve agent engines with BYOK keys and local LLMs, voice orchestration,
...
Questions, bugs, or want to show off your office? A community Slack is on the way; until then, open an issue.
...
> LimeZu is required by that licence** and must stay in place. The character cast is not LimeZu art. It
...
covers the code only. The LimeZu pixel-art tilesets are licensed separately (see above) and are
**not included in this repository**: LimeZu's licence does not permit redistributing the asset
pack, so `src/renderer/src/assets/tilesets/` here holds only a placeholder note. Official builds
bundle the licensed assets; a from-source build renders the office scene without its tileset art
unless you purchase the pack from [LimeZu](https://limezu.itch.io/moderninteriors) yourself.
