...
export interface HarnessConfig {
  ...
  /** What the user said they do (first onboarding screen). Mirrors src/main/config.ts. */
  ...
  autoMode: boolean;
  /** May the orchestrator ("Rudy") spin up agents on its own? Default FALSE,
...
/** The Sonnet model with the 1M-token context window — used for Rudy's prep
...
export const CRUSH_MODELS: ModelOption[] = [
  ...
  // An `ollama/*` slug would bypass the proxy (verify-crush NIT-2).
 ...
 *  high-context default for Rudy; other entries are curated quick-picks —
 ...
 *  Boss must remain on a provider with a working inbox drain; otherwise switching
 *  to a terminal-only provider would silently disable orchestration. */
export function modelProvidersForAgent(isBoss = false) {
  return AGENT_PROVIDER_PRESETS.filter((preset) =>
    preset.supportsModel && (!isBoss || preset.canReceiveInbox)
  );
}
