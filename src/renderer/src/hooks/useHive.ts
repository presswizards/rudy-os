...
// never fires → a boss stops draining mail and the floor stalls. So a 'working'
...
// After a boss/agent spawn, hold off the inbox-wake + queue-drain typers for this
...
// The first thing Rudy (boss) is told on a fresh spawn — orient him and put
// him to work running the floor. Kept terse and action-oriented.
const INITIAL_BOSS_PROMPT = [
  "You're online as Rudy, the orchestrator of the hive. Get oriented, then start running the floor:",
...
function enrichTaskPrompt(text: string): string {
  return [
    ...
    'self-contained prompt to Rudy via an outbox message with "to":"boss". Do not do the task yourself.)'
  ].join('\n');
}
 ...
 *   1. spawns the boss agent into Rudy's room when none is running,
...
export function useHive(config: HarnessConfig | null): void {
  ...
  // it verbatim (hive.ts normalize: `partial.id ?? ...`). One such id in boss's
  // inbox — `dev15-progress-canvas-v4` — sorts above EVERY `2026-*` timestamp and
  // never drains, so the "newest" id was frozen on it: Rudy was nudged once per
  ...
  // Rudy twice (the window between the listPtys check and spawnPty is racy).
  const bossSpawning = useRef(false);
  ...
  // Reactive so the assistant bootstrap (effect #1b) re-runs once Rudy is ready.
  ...
  useEffect(() => {
    ...
    void window.cth.hiveRegistry().then((reg) => {
      ...
      // Self-heal the clocked-out list from the registry: workers the hive
      // archived that the renderer lost (a mis-clicked clear, wiped storage)
      // come back as cards. Deliberately dismissed ids stay dismissed.
      const lost: Agent[] = Object.entries(reg.agents ?? {})
        // desk-* are closed side terminals at the boss, not hires — restoring
        // a dozen dead "Chat 2" cards would bury the actual workers.
        .filter(([id, e]) => e.archived === true && id !== BOSS_ID && !id.startsWith('desk-'))
        .map(([id, e]) => {
          const cast = OFFICE_CAST.find((m) =>
            m.name === (e.name || id).toLowerCase() || m.displayName.toLowerCase() === (e.name || id).toLowerCase());
          let h = 0;
          for (const ch of id) h = (h + ch.charCodeAt(0)) % SPAWN_ACCENTS.length;
          return {
            id,
            name: e.name || id,
            character: cast?.name ?? DEFAULT_CHARACTER,
            accent: SPAWN_ACCENTS[h],
            description: typeof e.role === 'string' && e.role.trim() ? e.role.trim() : 'a finished worker',
            project: (e.cwd || '').split(/[\\/]/).filter(Boolean).pop() || 'hive',
            tmuxTarget: '',
            cwd: e.cwd,
            status: 'idle',
            action: 'archived',
            progress: 0,
            command: '',
            provider: e.provider as Agent['provider'],
            isBoss: false,
            archived: true
          } as Agent;
        });
      if (lost.length > 0) useStore.getState().restoreArchivedFromHive(lost);
      const { agents, archivedAgents } = useStore.getState();
      for (const a of [...agents, ...archivedAgents]) {
        const next = preferredAgentRole(a.description, roles[a.id], !!a.isBoss);
  ...
  // 1) Bootstrap the boss agent (source of truth = live PTYs, to dodge restarts).
  useEffect(() => {
    if (!config?.onboardingComplete || !config.harnessHome) return;
    let cancelled = false;
    useStore.getState().setBossStatus('booting');
    const t = setTimeout(async () => {
      if (cancelled) return;
      const live = await window.cth.listPtys().catch(() => []);
      if (live.some((p) => p.id === BOSS_PTY)) { // already running — keep restored entry
        if (!cancelled) useStore.getState().setBossStatus('ready');
        return;
      }
      // Synchronous guard (no await between check and set) → exactly one spawn.
      if (cancelled || bossSpawning.current) return;
      bossSpawning.current = true;
      useStore.getState().removeAgent(BOSS_ID); // clear any stale restored entry

      const bossProvider = config.bossProvider ?? 'claude';
      const bossModel = config.bossModel;
      const command = buildSpawnCommand(config, bossModel, bossProvider);
      ...
      const res = await window.cth.spawnPty({
        ...
        cols: 100,
        rows: 30,
        // Restore Rudy's prior conversation across an app restart. His session
        ...
        resume: true,
        hive: { id: BOSS_ID, name: 'Rudy', provider: bossProvider, cwd: config.harnessHome!, isBoss: true, role: 'orchestrator (boss)' }
      });
      if (cancelled) { bossSpawning.current = false; return; }
      if (!res.ok) { bossSpawning.current = false; useStore.getState().setBossStatus('failed'); return; }
      const boss: Agent = {
        id: BOSS_ID,
        name: 'Rudy',
        character: 'rudy', // the original — every worker is one of his clones
        accent: 'lemon',
      ...
      useStore.getState().addAgent(boss);
      useStore.getState().setBossStatus('ready');

      // Kick Rudy off once his TUI is up. Always re-enable remote control so
      ...
      // a RESUMED Rudy already has his full context and must not be re-oriented
      ...
      // Rudy until he's settled. The live-PTY branch above skips this entirely.
      const resumedBoss = res.resumed === true;
      bootGraceUntil.current[BOSS_ID] = Date.now() + BOOT_GRACE_MS;
      void (async () => {
        try {
          const remoteCommand = remoteControlCommandForProvider(bossProvider, 'Rudy');
          if (remoteCommand) {
          ...
          if (!cancelled && !resumedBoss) {
            // A type-into-tui boss (Crush) can't ride its hive protocol on argv, so the
            ...
            if (res.seedPrompt) await submitToPty(BOSS_PTY, res.seedPrompt, bossProvider);
            await submitToPty(BOSS_PTY, INITIAL_BOSS_PROMPT, bossProvider);
          }
        } catch { /* PTY may have died during startup */ }
        finally { bootGraceUntil.current[BOSS_ID] = 0; }
      })();
    }, 1200);
  ...
  useEffect(() => {
    return window.cth.onHiveHookEvent((e) => {
      ...
      if (e.event === 'PreCompact') {
        ...
        // Only (1) is a real "needs you". Treating (2) as blocked made Rudy
        ...
        if (needsHuman && !idleWaiting) {
          // Only the boss agent escalates to the human; sub-agents are autonomous
          // and read as "waiting" (parked on boss, not on you).
          updateAgent(e.agentId, { status: self.isBoss ? 'blocked' : 'waiting' });
        } else {
  ...
  //     backgrounded boss gets none. This is the floor-wide, provider-agnostic backstop:
    ...
    const iv = setInterval(async () => {
      ...
      for (const a of agents) {
        try {
          ...
          // on the floor when the agent is boss.
  ...
  //     nudge can never jam onto one line. (boss-as-Crush is seeded in its own boot
  ...
  useEffect(() => {
    ...
    const iv = setInterval(() => {
      const { agents, updateAgent } = useStore.getState();
      for (const a of agents) {
        if (!a.ptyId || a.isBoss || !a.seedPrompt || seeded.current.has(a.id)) continue;
  ...
  // 5) Pipe inbound Slack messages into Rudy's queue. The main-process Slack
  //    webhook server pushes each verified message here via IPC; enqueueing to
  //    BOSS_ID lands it in Rudy's queue exactly as if the user had typed it
  ...
  useEffect(() => {
    ...
    return window.cth.onSlackMessage((msg) => {
      ...
      // by main, the authoritative source — is prepended ONLY to boss's working
      // instruction (what gets typed into his PTY), so the board stays readable
      // while every Slack-origin boss-session runs under the autonomy policy. When
      // main sends no preamble (older build), boss just gets the raw text.
      const instruction = msg.autonomyPreamble ? `${msg.autonomyPreamble}${text}` : undefined;
      useStore.getState().enqueueMessage(BOSS_ID, text, { slack, instruction });
  ...
  useEffect(() => {
    ...
    const offSpawn = window.cth.onHiveAgentSpawned?.((rec) => {
      ...
      // what makes "spawn one called Noor" land on the Noor avatar with
  ...
  // 7) Auto-revive wedged PTYs after the Mac sleeps/locks. The main-process
  ...
  useEffect(() => {
    ...
    const revive = async (deadId: string): Promise<void> => {
      ...
      try {
        ...
        const hive = a.isBoss
          ? { id: a.id, name: a.name, cwd, provider, isBoss: true, role: roleForHiveSpawn(a) }
          : a.isAssistant
