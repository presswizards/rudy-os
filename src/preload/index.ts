...
export interface HiveAgentMeta {
  ...
  cwd: string;
  isBoss?: boolean;
  /** Rudy's prep assistant — send-only; enriches prompts and forwards them. */
  isAssistant?: boolean;
}
...
export interface HiveTask {
  ...
  priority: number;
  createdAt: string;
  /** First-class human feedback: boss appends {q}, the harness UI fills {a};
 ...
 *  the sender aimed at "human" (now routed to the boss proxy) — cosmetic tint
...
export interface ScheduledMission {
  ...
  /** 'rudy' = filed by the boss via trigger-requests/. Mirrors main/config.ts. */
  origin?: 'rudy';
}
...
export interface HarnessConfig {
  ...
  /** What the user said they do on the first onboarding screen. */
  role?: 'developer' | 'founder' | 'product' | 'marketing' | 'ops' | 'curious';
  ...
  defaultModel?: string;
  /** Which provider+model powers the BOSS orchestrator ("Rudy"). Default
   *  'claude' / 'claude-opus-4-8'. Mirrors src/main/config.ts. */
  bossProvider?: AgentProvider;
  bossModel?: string;
  ...
  /** Realtime Rudy voice loop — true ONLY while a session holds the mic
   ...
   *  Tuned in Settings → Realtime Rudy; the cost cap stays the runaway guard. */
...
/** One live boss-triggered ephemeral worker, as shown in the Workers tab. */
...
const api = {
   ...
   *  keep running; Rudy is told to stop routing work to them. */
   ...
   *  {} for the whole floor. Backs Realtime Rudy's get_messages. The renderer
   ...
   *  archived agents. Backs Realtime Rudy's get_agent_detail / list_agents. */
  ...
  /** Fired when a validated hire manifest arrives via the rudyos://
  ...
  /** Start the closing-time protocol: the boss broadcasts shutdown, every worker
   *  saves its memory and ACKs, the boss concludes — then the app quits itself.
   *  Resolves with ok:false (+ error) when no boss agent is running. */
  ...
  // ─── Slack integration (Slack message → Rudy's queue) ─────────────────────
  ...
  /** Answer a held message. 'approved' RELEASES it to the hive (card + boss
  ...
  // Per-MCP-server credentials (GitHub PAT, DB connection string, search key) —
  // same write-only broker contract as providerKey*, keyed by catalog id.
  mcpKeySet: (req: { id: string; key: string }): Promise<{ ok: boolean; error?: string }> =>
    ipcRenderer.invoke('mcpKey:set', req),
  mcpKeyHas: (id: string): Promise<boolean> =>
    ipcRenderer.invoke('mcpKey:has', id),
  mcpKeyClear: (id: string): Promise<{ ok: boolean; error?: string }> =>
    ipcRenderer.invoke('mcpKey:clear', id),
  // One-click GitHub connect (OAuth device flow). Start opens the browser with
  // a short code on the clipboard; wait resolves once the user approves there
  // (the token is stored MAIN-side and never crosses this bridge).
  mcpOauthGithubAvailable: (): Promise<boolean> =>
    ipcRenderer.invoke('mcpOauth:githubAvailable'),
  mcpOauthGithubStart: (): Promise<{ ok: boolean; userCode?: string; verificationUri?: string; error?: string }> =>
    ipcRenderer.invoke('mcpOauth:githubStart'),
  mcpOauthGithubWait: (): Promise<{ ok: boolean; error?: string }> =>
    ipcRenderer.invoke('mcpOauth:githubWait'),
  // One-click Google connect (loopback OAuth): the browser opens, the user
  // approves, the app catches the redirect locally. Resolves when done.
  mcpOauthGoogleAvailable: (): Promise<boolean> =>
    ipcRenderer.invoke('mcpOauth:googleAvailable'),
  mcpOauthGoogleConnect: (): Promise<{ ok: boolean; error?: string }> =>
    ipcRenderer.invoke('mcpOauth:googleConnect'),
  // Realtime Rudy (voice orchestrator) — MAIN mints a short-lived EPHEMERAL token
  ...
  // the tiering, two-step verbal confirm, hard allowlist, and rudy-voice
  ...
  // human-speakable line Rudy relays; the rest is context for a toast/log.
