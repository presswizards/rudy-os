...
import { MCP_CATALOG, GITHUB_OAUTH_CLIENT_ID, GOOGLE_OAUTH_CLIENT_ID, GOOGLE_OAUTH_CLIENT_SECRET } from '../shared/mcpCatalog';
import { parseTriggerRequest } from './triggerRequests';
import {
...
// Fresh-funnel testing: point the WHOLE app (config, roster, secrets, hive
// pointer) at a throwaway data dir so onboarding can be walked as a brand-new
// user without touching the real profile. Dev-only by nature — set
// RUDY_USER_DATA=/some/tmp/dir before launch; unset = the normal profile.
if (process.env.RUDY_USER_DATA) {
  app.setPath('userData', process.env.RUDY_USER_DATA);
}
...
// Usage provider (Seam 1) — the INTEGRATION swap: The telemetry collector (#7)
...
// Rudy reads + the breaker beat, so guardrails + monitoring work even when the
// heartbeat mission is disabled (it ships off).
let fleetTimer: ReturnType<typeof setInterval> | null = null;
let breakerBeatTimer: ReturnType<typeof setInterval> | null = null;
// Feed the breaker's api_error-storm trip from the OTel api_error spans —
// The one breaker input with no on-branch source (telemetry.onApiError seam).
...
// HookServer needs BOTH: The control registry (HITL pause/gate/steer/halt via
// hook returns) AND the breaker (feed recordToolUse on each PostToolUse).
...
/** The PRIMARY window — the one running the hive/boss orchestration and the sink
...
/** A live boss-triggered ephemeral worker, tracked from spawn to teardown. */
interface WorkerRec {
 ...
 *  force-removed) when it holds unintegrated work — boss is the sole integrator. */
...
// Per-MCP-server credentials (GitHub PAT, DB connection string, …) live in the
// same encrypted write-only broker under their own prefix. Resolved MAIN-only at
// spawn (resolveMcpSecrets) and injected into the server's env by hive.ts.
const mcpKeyRef = (id: string): string => `mcpkey:${id}`;
/** Catalog ids that take a credential (secret-tier entries with an env spec). */
const mcpKeyableIds = (): string[] => MCP_CATALOG.filter((e) => e.spec.env).map((e) => e.id);
/** Broker → {catalogId: plaintext} for every keyed server with a stored secret.
 *  Google's email-calendar entry stores a token JSON; it is refreshed here so
 *  the agent always spawns with a LIVE access token, not a stale one. */
const resolveMcpSecrets = async (): Promise<Record<string, string>> => {
  const out: Record<string, string> = {};
  for (const id of mcpKeyableIds()) {
    const s = integrations.getSecret(mcpKeyRef(id));
    if (!s) continue;
    out[id] = id === 'email-calendar' ? await freshGoogleAccessToken(s) : s;
  }
  return out;
};
...
/** Send an inform to the boss agent (the human's proxy). The ephemeral-worker
 *  controller uses this to surface every terminal failure AND to carry the Slack
 *  {channel,thread_ts} so boss can post a 'couldn't complete' reply — closing the
 *  Slack loop (the success path is the worker replying in-thread itself). */
function informBoss(subject: string, body: string, slack?: { channel: string; thread_ts: string }): void {
  try {
    const slackLine = slack
      ...
      : '';
    hive.send({ to: 'boss', act: 'inform', subject, body: body + slackLine }, 'ephemeral-worker');
  } catch (e) {
    console.error('[worker] informBoss failed:', e);
  }
}

/** Gated worktree teardown for an ephemeral worker: remove it ONLY when it holds no
 *  unintegrated work; otherwise leave it (and its branch) in place and ping boss, the
  ...
  for (const m of missions) {
    ...
    const fire = (): void => {
      try {
        ...
        // their prior behaviour, including the historical empty-body send (N1).
 ...
 *  WITHOUT archiving (e.g. the pre-acc13a3 'assistant' entry). Left as-is
 *  they have no live PTY, so the breaker beat steers them and the steer bounces to
 *  BOSS as a requires_reply BOSS can't clear → inbox flood.
 *
 *  "No live PTY" = ptyForAgent(id) === undefined (ptyToAgent is populated only at
 *  spawn and pruned on teardown). Boss is never archived. A user's real agents are
...
function archiveOrphanedAgents(): void {
  ...
  try {
    ...
    for (const [id, a] of Object.entries(reg.agents)) {
      ...
      if (id === reg.bossId) continue;        // boss is never archived
...
function looksStuck(windowMs: number): boolean {
  ...
  const now = Date.now();
  for (const [id, a] of Object.entries(reg.agents)) {
    if (a.archived || id === reg.bossId) continue;
...
/** Bounded digest for boss — paths + counts, never full files (reference-passing,
 *  #6.2). A few hundred tokens at most. */
function buildHeartbeatDigest(quietMs: number, actionable = 0): string {
  const reg = hive.registry();
  const active = Object.entries(reg.agents).filter(([id, a]) => !a.archived && id !== reg.bossId);
  ...
  // inbox, not because the floor went quiet, and boss must read it now.
 ...
 *  boss. Everything else (a worker agent id, 'webhook', a human reply) is real
 *  mail boss must act on. Kept narrow so any future real sender counts by default. */
const SYSTEM_SENDERS = new Set(['heartbeat', 'scheduler', 'breaker', 'system']);

/** Count of UNREAD actionable messages in boss's inbox — real agent/human mail,
 ...
 *  "loud", so boss was never re-engaged until everything else went idle. */
function bossActionableInboxCount(): number {
  try {
    const bossId = hive.registry().bossId;
    if (!bossId) return 0;
    return hive.inbox(bossId).filter((m) => !SYSTEM_SENDERS.has(m.from)).length;
  } catch { return 0; }
}

/** Re-engage a quiet floor: drop a durable digest into boss's inbox. We never
 *  type directly into boss's PTY here — if he's busy that would jam mid-step. The
 *  inbox message is delivered by the renderer's busy-aware inbox-wake (it nudges
 *  boss to read his inbox only once he's idle), so the heartbeat defers around a
 *  working boss instead of interrupting him. */
function reengageBoss(digest: string): void {
  if (!hive.enabled()) return;
  hive.send({ to: 'boss', act: 'request', subject: 'Heartbeat', body: digest }, 'heartbeat');
}
 ...
 *  escalation. Boss is in the LEDGER (cost visibility) but NOT the breaker inputs
 *  (the heartbeat manages boss; we never auto-steer/kill the orchestrator). */
function runBreakerBeat(progressWindowMs: number): void {
  ...
  for (const [id, a] of Object.entries(reg.agents)) {
    ...
    // a stale name) or any orphaned entry left archived:false with NO live PTY would
    // otherwise be steered, and that steer bounces to BOSS as a requires_reply BOSS
    // can't clear → inbox flood. ptyForAgent(id) === undefined means no live PTY.
    // Boss is exempt from this orphan check (it keeps its own flow + the bossId skip
    ...
    if (id !== reg.bossId && !ptyForAgent(id)) continue;
    ...
    if (sample?.sessionId) hive.appendCostLedger(sample); // ledger covers everyone incl. boss
    ...
    if (id === reg.bossId) continue;            // breaker skips boss
  ...
  for (const d of breaker.tick(inputs, now)) {
    ...
    if (d.action === 'steer') {
      hive.send({ to: d.state.agentId, act: 'request', subject: 'Circuit breaker: steer',
        body: `Automated guardrail: ${reason}. Re-check your approach, if you're looping or stuck, STOP repeating, summarize what you've tried, and ask boss for direction.` }, 'breaker');
    } else if (d.action === 'constrain') {
      hive.send({ to: d.state.agentId, act: 'request', subject: 'Circuit breaker: constrain',
        body: `Automated guardrail escalated: ${reason}. Stop active work now: switch to read-only/plan, write a short plan of your next step, and send it to boss for sign-off BEFORE running more tools.` }, 'breaker');
...
/** Build + write the live fleet snapshot Rudy reads (`<hive>/fleet.json`).
    ...
    const agents = Object.entries(reg.agents)
      ...
      .map(([id, a]) => {
        ...
        return {
          id,
          name: a.name,
          role: a.role ?? (a.isBoss ? 'orchestrator' : 'agent'),
          cwd: a.cwd,
          isBoss: !!a.isBoss,
...
function armHeartbeat(m: ScheduledMission): void {
  ...
  const beat = (): void => {
    ...
    try {
      ...
      // Re-engage boss when the floor is quiet OR when real agent/human mail is
      // waiting in boss's inbox — the latter is independent of floor-quiet so a
      // worker's reply doesn't sit unread while other agents keep the floor busy.
      const actionable = bossActionableInboxCount();
      if (isFloorQuiet(quiet) || actionable > 0) {
        reengageBoss(buildHeartbeatDigest(quiet, actionable));
...
// ─── Slack webhook server (Slack message → Rudy's queue) ──────────────────
 ...
 *  path. Prepended (server-side, authoritatively) to the working instruction boss
 *  reads for any Slack-origin request: there is no interactive human at the
 *  keyboard, so boss must route fast, delegate WITH the exact reply command (so the
 *  worker posts its real result back into THIS thread itself), stay autonomous,
 *  and only block on enumerated high-severity actions. Prepended to boss's PROMPT
...
/** Absolute path to the bundled `rudy-slack-reply.cjs` helper. Packaged: under
...
function slackReplyScriptPath(): string {
  return app.isPackaged
    ? join(process.resourcesPath, 'rudy-slack-reply.cjs')
...
async function startSlackServer(): Promise<{ ok: boolean; url?: string; error?: string }> {
  ...
  slackServer = new SlackWebhookServer({
    ...
    onMessage: async (m) => {
      ...
      // prepends ONLY to boss's working instruction (his PTY prompt), keeping the
      ...
      // resolved helper path — boss hands the worker an exact reply command.
 ...
 *  so `rudy-slack-reply.cjs` can reach it. The bot token is read lazily from config
...
/**
 * Create the stamped kanban card for an inbound message and route it to boss.
 *
 ...
 * same boss request an auto-allowed message would have, or the two paths drift
 ...
 * written. The boss routing is best-effort: the card already exists and is
 ...
 * exactly as the single-endpoint server always did — card, boss request, capability
...
// ─── Shareable hires: rudyos:// deep link + file import ──────────────
...
if (process.defaultApp) {
  if (process.argv.length >= 2) {
    app.setAsDefaultProtocolClient('rudyos', process.execPath, [resolve(process.argv[1])]);
  }
} else {
  app.setAsDefaultProtocolClient('rudyos');
}
...
if (!gotInstanceLock) {
  ...
  app.on('second-instance', (_evt, argv) => {
    ...
    const link = argv.find((a) => a.startsWith('rudyos://'));
...
function createWindow(opts: { floor?: boolean } = {}): BrowserWindow {
  ...
  const win = new BrowserWindow({
    ...
    title: isFloor ? 'Rudy OS · Floor' : 'Rudy OS',
  ...
  // Realtime Rudy voice session (`realtimeVoiceEnabled`, flipped on by the
...
/** Spawn options shared by the `pty:spawn` IPC handler and the boss-triggered
 ...
 *  it can ALSO be invoked by the boss-triggered ephemeral-worker watcher (which has
...
async function spawnAgentCore(opts: AgentSpawnOptions, owner: Electron.WebContents | null): Promise<{ ok: boolean; error?: string; cwd?: string; worktreePath?: string; resumeNotFound?: boolean; resumed?: boolean; seedPrompt?: string }> {
  ...
  // the boss-triggered ephemeral-worker watcher), so it is where a user-typed
    ...
    try {
      const inj = await hive.ensureAgent(
        { ...opts.hive, cwd: opts.cwd, provider },
        {
          ...
          // Broker credentials for consented keyed servers — resolved here in
          // MAIN, handed to hive.ts to inject into each server's env. Without
          // this, an enabled GitHub/DB/search server shipped a blank token.
          mcpSecrets: await resolveMcpSecrets(),
  ...
  // so the renderer can skip re-orienting a boss/assistant that resumed its thread.
  let didResume = false;
  ...
  if (opts.hive && claudeProvider) {
    ...
    // BOSS is special-cased: it has its own engine config (bossProvider/bossModel), so
    // modelForRole resolves it and that wins over the worker-oriented defaultModel.
    if (!args.includes('--model')) {
      const m = opts.hive.isBoss
        ? modelForRole(opts.hive, cfg)
        : cfg.defaultModel ?? modelForRole(opts.hive, cfg);
      if (m) args.push('--model', m);
    }
    // Name the Remote Control session after the agent (Rudy, Theo, Dev1…) so it
  ...
  // use their own login, so they skip this. Guardrails #3/#4/#5.
  if (opts.hive && (provider === 'opencode' || provider === 'crush' || provider === 'pi' || provider === 'qwen')) {
    const cfg = readConfig();
    const extra: Record<string, string> = {};
    // 1) BYOK keys — LEAST-PRIVILEGE (NIT-2): inject ONLY the key for the
    ...
    for (const backend of backends) {
      ...
      // GEMINI_API_KEY — inject both so google/* authenticates (NIT #1).
    ...
    if (provider === 'opencode') {
      ...
      if (baseUrl) {
        ...
        // 'local' (verify-opencode MUST-FIX #2).
...
// ─── IPC: integrations (Phase 2 registry — backend for the Settings UI) ──────
...
// ─── IPC: per-MCP-server credentials (write-only, same broker) ───────────────
// The Connections quick-connect list stores each keyed server's credential under
// `mcpkey:<catalog id>`. Same contract as providerKey: set + boolean-has +
// clear; the plaintext never crosses back over IPC (materialized at spawn).
ipcMain.handle('mcpKey:set', (_evt, payload: unknown) => {
  const p = (payload ?? {}) as { id?: unknown; key?: unknown };
  if (typeof p.id !== 'string' || !mcpKeyableIds().includes(p.id)) return { ok: false, error: 'unknown server' };
  if (typeof p.key !== 'string' || !p.key) return { ok: false, error: 'key required' };
  return integrations.setSecret(mcpKeyRef(p.id), p.key);
});
ipcMain.handle('mcpKey:has', (_evt, id: unknown) =>
  typeof id === 'string' ? integrations.hasSecret(mcpKeyRef(id)) : false);
ipcMain.handle('mcpKey:clear', (_evt, id: unknown) => {
  if (typeof id !== 'string' || !mcpKeyableIds().includes(id)) return { ok: false, error: 'unknown server' };
  try { integrations.deleteSecret(mcpKeyRef(id)); return { ok: true }; }
  catch (e) { return { ok: false, error: e instanceof Error ? e.message : String(e) }; }
});
// ─── IPC: one-click GitHub connect (OAuth device flow, no server needed) ─────
// The `gh` CLI's login style: ask GitHub for a short user code, open the
// approval page in the browser with the code on the clipboard, poll until the
// user approves, then store the token in the same broker slot the paste field
// uses. Gated on GITHUB_OAUTH_CLIENT_ID being set (a free OAuth App the
// product owner registers once); without it the renderer never shows the button.
let ghDeviceFlow: { deviceCode: string; interval: number; expiresAt: number } | null = null;
ipcMain.handle('mcpOauth:githubAvailable', () => GITHUB_OAUTH_CLIENT_ID.length > 0);
ipcMain.handle('mcpOauth:githubStart', async () => {
  if (!GITHUB_OAUTH_CLIENT_ID) return { ok: false, error: 'no OAuth app configured' };
  try {
    const res = await fetch('https://github.com/login/device/code', {
      method: 'POST',
      headers: { Accept: 'application/json', 'Content-Type': 'application/json' },
      body: JSON.stringify({ client_id: GITHUB_OAUTH_CLIENT_ID, scope: 'repo read:org' })
    });
    ...
    if (!j.device_code || !j.user_code || !j.verification_uri) {
      return { ok: false, error: j.error ?? 'GitHub did not return a device code' };
    }
    ghDeviceFlow = {
      deviceCode: j.device_code,
      interval: Math.max(5, j.interval ?? 5),
      expiresAt: Date.now() + Math.min((j.expires_in ?? 900), 900) * 1000
    };
    clipboard.writeText(j.user_code);
    void shell.openExternal(j.verification_uri);
    return { ok: true, userCode: j.user_code, verificationUri: j.verification_uri };
  } catch (e) {
    return { ok: false, error: e instanceof Error ? e.message : String(e) };
  }
});
// Resolves when the user approves in the browser (or the code expires). Runs
// the whole poll loop MAIN-side so the token never crosses to the renderer.
ipcMain.handle('mcpOauth:githubWait', async () => {
  const flow = ghDeviceFlow;
  if (!flow) return { ok: false, error: 'no connect in progress' };
  let intervalMs = flow.interval * 1000;
  while (Date.now() < flow.expiresAt) {
    await new Promise((r) => setTimeout(r, intervalMs));
    if (ghDeviceFlow !== flow) return { ok: false, error: 'superseded' };
    try {
      ...
      if (j.access_token) {
        ghDeviceFlow = null;
        return integrations.setSecret(mcpKeyRef('github-token'), j.access_token);
      }
      if (j.error === 'slow_down') { intervalMs += 5000; continue; }
      if (j.error === 'authorization_pending') continue;
      if (j.error === 'expired_token' || j.error === 'access_denied') {
        ghDeviceFlow = null;
        return { ok: false, error: j.error === 'access_denied' ? 'you declined in the browser' : 'the code expired, start again' };
      }
    } catch { /* transient network error — keep polling until expiry */ }
  }
  ghDeviceFlow = null;
  return { ok: false, error: 'the code expired, start again' };
});
// ─── IPC: one-click Google connect (loopback OAuth, no server needed) ────────
// Desktop-app pattern: bind a one-shot listener on 127.0.0.1, send the browser
// to Google's consent page with that loopback as the redirect, catch the code,
// exchange it for tokens, store them in the broker. The refresh token lets
// resolveMcpSecrets mint a fresh access token at every agent spawn.
const GOOGLE_SCOPES = 'https://www.googleapis.com/auth/gmail.modify https://www.googleapis.com/auth/calendar';
ipcMain.handle('mcpOauth:googleAvailable', () => GOOGLE_OAUTH_CLIENT_ID.length > 0 && GOOGLE_OAUTH_CLIENT_SECRET.length > 0);
ipcMain.handle('mcpOauth:googleConnect', async () => {
  if (!GOOGLE_OAUTH_CLIENT_ID || !GOOGLE_OAUTH_CLIENT_SECRET) return { ok: false, error: 'no OAuth app configured' };
  try {
    const { createServer } = await import('node:http');
    // One-shot loopback listener: resolves with the auth code AND the exact
    // redirect URI it was issued for (the exchange must repeat it verbatim).
    const { code, redirect } = await new Promise<{ code: string; redirect: string }>((resolvePromise, rejectPromise) => {
      let redirectUri = '';
      const server = createServer((req, res) => {
        ...
        res.end('<body style="font-family:monospace;background:#141414;color:#E8E6E3;display:flex;align-items:center;justify-content:center;height:100vh"><div>' +
          (c ? 'Connected. You can close this tab and go back to Rudy OS.' : 'Connection cancelled. You can close this tab.') +
          '</div></body>');
        server.close();
        clearTimeout(timer);
        if (c) resolvePromise({ code: c, redirect: redirectUri });
        else rejectPromise(new Error(err ?? 'you declined in the browser'));
      });
      const timer = setTimeout(() => { server.close(); rejectPromise(new Error('timed out waiting for the browser approval')); }, 5 * 60 * 1000);
    ...
    if (!j.access_token) return { ok: false, error: j.error ?? 'Google did not return a token' };
    return integrations.setSecret(mcpKeyRef('email-calendar'), JSON.stringify({
...
/** Google access tokens live ~1h. When the stored email-calendar secret is a
 *  token JSON with a refresh token, mint a fresh access token; agents then get
 *  a live GOOGLE_OAUTH_TOKEN at every spawn instead of a stale one. */
async function freshGoogleAccessToken(stored: string): Promise<string> {
  try {
    const t = JSON.parse(stored) as { access_token?: string; refresh_token?: string; expires_at?: number };
    if (!t.access_token) return stored;                       // a hand-pasted raw token
    if (!t.refresh_token || !GOOGLE_OAUTH_CLIENT_ID) return t.access_token;
    ...
    if (j.access_token) {
      integrations.setSecret(mcpKeyRef('email-calendar'), JSON.stringify({
        ...t, access_token: j.access_token, expires_at: Date.now() + (j.expires_in ?? 3600) * 1000
      }));
...
ipcMain.handle('config:update', (_evt, patch: Partial<HarnessConfig>) => {
  ...
  const wasOnboarded = readConfig().onboardingComplete === true;
  ...
  // The first-run wizard just finished: one anonymous event with the role the
  // user picked, the copy register and the orchestrator engine (TELEMETRY.md).
  if (!wasOnboarded && next.onboardingComplete === true) {
    analytics.track('onboarding_completed', {
      role: next.role ?? 'unknown',
      audience: next.audience ?? 'unknown',
      provider: next.bossProvider ?? 'unknown'
    });
  }
  ...
  // built per spawn, so flipping the toggle reaches boss the next time he starts.
...
ipcMain.handle('config:changeHome', async (_evt, payload: unknown) => {
  ...
  try { stopTriggerRequestWatcher(); } catch (e) { console.error('[changeHome] stopTriggerRequestWatcher:', e); }
 ...
 *  and the closing-time conclusion (after the boss confirmed the floor saved). */
function teardownAndQuit(): void {
  ...
  try { stopTriggerRequestWatcher(); } catch (e) { console.error('[quit] stopTriggerRequestWatcher:', e); }
...
// The third quit-dialog button. The boss broadcasts closing time, every worker
// saves its memory and ACKs, the boss concludes with CLOSING-TIME-COMPLETE —
...
ipcMain.handle('app:resetAll', () => {
  ...
  try { stopTriggerRequestWatcher(); } catch (e) { console.error('[reset] stopTriggerRequestWatcher:', e); }
  ...
  // Erase the hive (Rudy's + every agent's memory, inboxes, tasks, board,
...
// (Realtime Rudy's get_agent_detail / list_agents). One read that joins
// everything the office-floor sidebar + telemetry know per agent: the registry
// record (name/role/provider/cwd/status/archived/isBoss/isAssistant/sessionId/
...
// is live-only) so Rudy can speak to inactive agents — their cwd and memory
...
ipcMain.handle('hive:agentDirectory', () => {
  if (!hive.enabled()) return { bossId: null, agents: [] };
  const reg = hive.registry();
  const snap = telemetry.snapshot();
  const usageById = new Map(snap.usage.map((u) => [u.agentId, u]));
  const now = Date.now();
  ...
  return { bossId: reg.bossId, agents };
});
...
// before the policy lands; he produces, this lane consumes.
 ...
 * would have taken (card + boss request), then the entry flips. 'rejected' just
...
// ─── IPC: Realtime Rudy (voice orchestrator — ephemeral token mint, rt-1) ──
...
// ─── IPC: Realtime Rudy voice ACTIONS (rt-5, Phase 2) ─────────────────────
// Thin adapters over the SAME main fns the boss PTY already uses. ALL of the safety
// spine — soft-vs-destructive tiering, the two-step verbal echo-back confirm, the
// distinct-token rule, the hard allowlist (kill-boss / mass-ops forbidden), and the
// rudy-voice attribution — lives in ./realtimeActions. This site only injects
// the existing functions; it adds NO new orchestration logic.
// ─── IPC: Realtime Rudy completion watcher (rt-12, Phase 2) ───────────────
// The net-new engine (realtimeCompletionWatcher.ts) detects a voice-dispatched
// task finishing (card→done OR a done-reply in rudy-voice's inbox) and EMITS it;
// I own the seam — inject the hive read deps, push completions to the live session
// (so Rudy speaks them unprompted), and bridge waitFor / queue-drain over IPC.
const completionWatcher = initCompletionWatcher({
  ...
  // Voice dispatches go out as from:rudy-voice, so assignee done-replies land here.
  readInbox: () => {
    // Voice dispatches go out from:rudy-voice, so done-replies normally land in its
    // inbox — but an assignee may address boss out of habit. Merge both inboxes (de-dupe
    // by id) so a boss-addressed completion isn't missed; the detector filters by sender.
    try {
      const mv = hive.inbox('rudy-voice') as unknown as InboxMessage[];
      const bossId = hive.registry().bossId;
      const boss = bossId ? (hive.inbox(bossId) as unknown as InboxMessage[]) : [];
      const seen = new Set<string>();
      return [...mv, ...boss].filter((m) => !!m?.id && !seen.has(m.id) && seen.add(m.id) !== undefined);
    } catch {
      return [];
    }
  },
  onNotify: (evt) => { try { if (Notification.isSupported()) new Notification({ title: 'Rudy', body: evt.summary }).show(); } catch { /* best-effort */ } }
});
...
// ─── boss-triggered ephemeral Slack workers ──────────────────────────────────
// boss drops a spawn-request JSON into HIVE_ROOT/spawn-requests/; MAIN polls that
...
// unintegrated work. Every terminal failure informs boss WITH the Slack coords so
// boss closes the Slack loop; the success path is the worker replying in-thread.

/** A spawn-request boss drops into HIVE_ROOT/spawn-requests/<id>.json. boss authors
...
/** HIVE_ROOT/spawn-requests — the queue dir boss drops requests into. */
...
/* ─── Trigger requests: HIVE_ROOT/trigger-requests → config.missions ─────────
   The boss cannot reach the renderer's trigger IPC from a pty, so schedules get
   the same file door workers got. One JSON in, validated by
   main/triggerRequests.ts, merged into config.missions (id = trg-<filename> so a
   re-drop UPDATES rather than duplicates), archived to .done/.failed. Unlike
   spawn-requests this is NOT gated on orchestratorMaySpawn: a schedule only
   queues prompts, it spends nothing until it fires, and every one lands in the
   SCHEDULES panel with a BY RUDY chip and a working off switch. */
const TRIGGER_REQ_TICK_MS = 2000;
let triggerReqTimer: ReturnType<typeof setInterval> | null = null;
let triggerReqTickRunning = false;

function triggerRequestsDir(): string | null {
  const root = hive.root();
  return root ? join(root, 'trigger-requests') : null;
}

function archiveTriggerRequest(filePath: string, sub: '.done' | '.failed', note?: string): void {
  const queue = triggerRequestsDir();
  try {
    if (!queue) throw new Error('no hive root');
    const dir = join(queue, sub);
    mkdirSync(dir, { recursive: true });
    renameSync(filePath, join(dir, basename(filePath)));
    if (note) writeFileSync(join(dir, basename(filePath) + '.error.txt'), note, 'utf8');
  } catch (e) {
    try { unlinkSync(filePath); } catch { /* noop */ }
    console.error('[trigger-req] archive failed:', e);
  }
}

function triggerRequestTick(): void {
  if (triggerReqTickRunning) return;
  triggerReqTickRunning = true;
  try {
    const dir = triggerRequestsDir();
    ...
    for (const f of files) {
      ...
      try {
        ...
        archiveTriggerRequest(filePath, '.failed', `unreadable JSON: ${e instanceof Error ? e.message : String(e)}`);
        continue;
      }
      const id = `trg-${basename(f, '.json')}`;
      const parsed = parseTriggerRequest(raw, id);
      if (!parsed.ok) {
        console.warn('[trigger-req]', f, 'rejected:', parsed.error);
        archiveTriggerRequest(filePath, '.failed', parsed.error);
        continue;
      }
      const current = readConfig().missions ?? [];
      const existing = current.find((m) => m.id === id);
      const next = existing
        // Re-drop = update in place; the scheduler's lastFiredAt survives.
        ? current.map((m) => (m.id === id ? { ...parsed.mission, lastFiredAt: m.lastFiredAt } : m))
        : [...current, parsed.mission];
      writeConfig({ missions: next });
      syncMissions();
      try { liveWebContents()?.send('missions:updated'); } catch { /* window gone */ }
      console.log('[trigger-req] armed', id, `(${parsed.mission.label})`);
      archiveTriggerRequest(filePath, '.done');
    }
  } catch (e) {
    console.error('[trigger-req] tick error:', e);
  } finally {
    triggerReqTickRunning = false;
  }
}

function startTriggerRequestWatcher(): void {
  if (triggerReqTimer || !hive.enabled()) return;
  const dir = triggerRequestsDir();
  if (dir) { try { mkdirSync(dir, { recursive: true }); } catch { /* noop */ } }
  triggerReqTimer = setInterval(triggerRequestTick, TRIGGER_REQ_TICK_MS);
}

function stopTriggerRequestWatcher(): void {
  if (triggerReqTimer) { clearInterval(triggerReqTimer); triggerReqTimer = null; }
}
 ...
 *  request, missing CLI, spawn error) archive to .failed and inform boss WITH the
 *  Slack coords so boss can post a 'couldn't start' reply. On success the worker is
...
async function processSpawnRequest(filePath: string): Promise<void> {
  ...
  try {
    ...
    informBoss('[worker spawn rejected] unparseable request', `Could not parse spawn-request ${basename(filePath)}${String(e)}`);
  ...
  const fail = (reason: string): void => {
    informBoss(`[worker spawn rejected] ${reason}`, `Spawn-request ${basename(filePath)} rejected: ${reason}.`, slack);
    archiveRequest(filePath, '.failed');
  };
  ...
  // so we never run the cc49e1e install banner here — we reject and tell boss.
  ...
  // A boss-hired worker is a MAIN-initiated spawn, so the renderer would never
  ...
  // reply command + autonomy policy. `from: boss` so the worker treats it as a boss
  ...
  try {
    ...
    const suffix = `\n\n[CAPABILITIES] Before you start, consult your capability catalog, run the \`/capabilities\` skill (or read \`$AGENT_DIR/.claude/skills/capabilities/SKILL.md\`). It lists your temporal date-range skills (\`/today\`, \`/last30Days\`, \`/lastQuarter\`, …) and the integrations available to you (reached via the loopback broker) and how to call each. For any time-scoped work, resolve the dates with those skills instead of computing them by hand.\n\n[WORKER COMPLETION] When finished, signal done by sending ONE outbox message to boss with "act":"done" and a short result summary, that releases this ephemeral worker (terminal closed; your branch is handed to boss). Do NOT push to any remote; boss is the sole integrator.`;
    hive.send({ to: workerId, conversation: `worker-${reqId}`, act: 'request', subject: meta.name, body: `${prefix}${objective}${suffix}` }, 'boss');
  } catch (e) {
 ...
 *  KEEPS it (never discards un-integrated work — boss is the sole integrator). Runs
...
async function gcPreservedWorktrees(): Promise<void> {
  ...
  try {
    for (const [key, e] of [...preservedWorktrees]) {
      ...
      // (a) Worktree already gone (removed at clean teardown, or boss removed it by
...
async function ephemeralWorkerTick(): Promise<void> {
  ...
  try {
    ...
    //     no hive archive, no hive:agentArchived, frozen floor cards, and boss
    ...
    //     than being eaten and failed for a reason boss never asked about.
...
function bootstrapHiveServices(): void {
  ...
  startTriggerRequestWatcher(); // poll HIVE_ROOT/trigger-requests → schedules Rudy files himself
 ...
 *  fleet snapshot Rudy reads (~8s) + the breaker/cost-ledger beat (~30s).
...
function onSystemResume(reason: string): void {
  ...
  // scheduler→boss path recovered (it injects straight into boss's inbox), while
  // every agent's outbox silently stopped draining, so boss→worker and
...
app.whenReady().then(() => {
  // Realtime Rudy mic-gate hygiene (rt-8 / rt-10 nit): the voice session
  ...
  const startupHireLink = process.argv.find((a) => a.startsWith('rudyos://'));
